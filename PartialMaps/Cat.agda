...
module PartialMaps.Cat {i j}(X : Cat {i}{j})(M : StableSys X) where
...
record Span (A B : Obj) : Set (i ⊔ j) where
  constructor span
  field A'    : Obj
        mhom  : Hom A' A
        fhom  : Hom A' B
...
record _~Span~_ {A B}(mf ng : Span A B) : Set (i ⊔ j) where
  constructor spaneq 
...
qspan : ∀ A B → Quotient {j = j} (Span A B) Span~EqR
...
compat : ∀{A B}(C : QSpan A B → Set j) → 
         ((mf : Span A B) → C (abs mf)) → Set (i ⊔ j)
compat {A}{B} = Quotient.compat (qspan A B)

lift : ∀{A B}(C : QSpan A B → Set j)
...
compat₃ : ∀{A B A' B' A'' B''}
          (C : QSpan A B → QSpan A' B' → QSpan A'' B'' → Set j)
          (f : (mf : Span A B)(ng : Span A' B')(lh : Span A'' B'') → 
                C (abs mf) (abs ng) (abs lh)) → Set (i ⊔ j)
compat₃ {A}{B}{A'}{B'}{A''}{B''} = 
