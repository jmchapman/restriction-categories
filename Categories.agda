...
-- every idempotent splits
  ...
  record SplitMap (ide ide' : Idem) : Set where
    open Idem ide
    open Idem ide' renaming (E to E' ; e to e' ; law to law')
    field imap : Hom E E'
  ...
  splitcomp : {ide ide' ide'' : Idem} (f' : SplitMap ide' ide'') →
  ...
  splitidr : {ide ide' : Idem}{f : SplitMap ide ide'} → 
