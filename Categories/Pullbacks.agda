...
module Categories.Pullbacks {i j}(X : Cat {i}{j}) where
...
record Square {X Y Z}(f : Hom X Z)(g : Hom Y Z) : Set (i ⊔ j) where
  constructor square
  field W    : Obj
        h    : Hom W X
        k    : Hom W Y
...
record SqMap {X Y Z : Obj}{f : Hom X Z}{g : Hom Y Z}
             (sq' sq : Square f g) : Set (i ⊔ j) where
  constructor sqmap
  open Square
...
record Pullback {X Y Z}(f : Hom X Z)(g : Hom Y Z) : Set (i ⊔ j) where
