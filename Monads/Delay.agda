...
mutual
  ~refl  : ∀ {i X} (c : Delay ∞ X) → c ~⟨ i ⟩~ c
  ~refl (now x)   = ~now
  ~refl (later c) = ~later (∞~refl c)

  ∞~refl : ∀ {i X} (c : ∞Delay ∞ X) → c ∞~⟨ i ⟩~ c
  ~force (∞~refl c) = ~refl (force c)

-- Symmetry

mutual
  ~sym : ∀ {i X} {c c' : Delay ∞ X} → c ~⟨ i ⟩~ c' → c' ~⟨ i ⟩~ c
  ~sym ~now       = ~now
